...
def distribute_files(data):
    ...
    return splits, global_counts, targets
...
def print_stats(splits, global_counts, targets):
...
def main():
    ...
    # 2. Stratify aiming for instance-level targets
    splits, global_counts, targets = distribute_files(data)

    # 3. Copy
    copy_files(splits)

    # 4. Create YAML (New Step)
    create_yaml()

    # 5. Report
    print_stats(splits, global_counts, targets)
