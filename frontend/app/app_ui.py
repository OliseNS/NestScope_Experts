...
def load_species_list():
    ...
    Returns a list of tuples (species_code, species_name).
    """
    try:
        species_map = {}
...
def extract_crops_from_detections(image_bytes, detections, num_crops=5):
    """
    Extract random crops from detected birds.
    Returns list of (crop_image_bytes, detection_info) tuples.
    """
    try:
        # Decode image
...
with cv_tab:
    ...
    if image_to_process:
        ...
        if result and "error" not in result:
            ...
            # Download and Correct AI buttons
            if annotated_base64:
                col1, col2 = st.columns(2)
                with col1:
                    st.download_button(
            ...
            if bird_count > 0:
                ...
                if crops:
                    ...
                    # Initialize session state for crop identifications
